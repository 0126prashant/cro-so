...
# Specify the creator ID you want to retrieve data for
creator_id = "65f19958b02b4a07815ce117"

# Query the website_info_collection to find the document with the specified creator ID
website_documents = website_info_collection.find_one({"creatorID": creator_id})
...
# Convert documents to a dictionary with creatorID as the key and process URLs
for doc in website_documents:
    # Process each mobile URL
    for mobile_img in doc['mobile']:
        for key, img_url in mobile_img.items():
            mobile_urls.append(img_url)
    
    # Process each desktop URL
    for desktop_img in doc['desktop']:
        for key, img_url in desktop_img.items():
            desktop_urls.append(img_url)
