...
function generatePdfWithJSPDF() {
  ...
  html2pdf()
    ...
    .then(function (pdfBlob) {
      ...
      fetch("http://localhost:8080/upload-pdf", {
        method: "POST",
        body: formData,
      })
...
//         feedbackContent += `<div>${i}. ${item.cro_feedback[`advice_heading_${i}`]}: ${item.cro_feedback[`advice_${i}`]}</div>`;
//       }
//     }
//     if (item.cro_feedback.super_cro_tip_advice) {
//       feedbackContent += `<div>Super CRO Tip: ${item.cro_feedback.super_cro_tip_advice}</div>`;
//     }
...
//   // Helper function to create and append item display
...
//     itemImage.style.height = isMobile ? "700px" : "600px";
...
//   // Create an array of promises for each desktop item
...
//       // Create and append desktop item display
//       parentDiv.appendChild(createItemDisplay(desktopItem, false));

//       // Create and append mobile item display if available
//       if (index < mobileData.length) {
//         parentDiv.appendChild(createItemDisplay(mobileData[index], true));
//       }

//       // Append footer here if needed
//       parentDiv.appendChild(generateFooter());

//       // Append the newly created parentDiv to the main container
//       main.appendChild(parentDiv);

//       resolve(); // Resolve the promise when the item is appended
//     });
//   });

//   // Handle any additional mobile items not matched with a desktop item
...
//         // Create and append additional mobile items with alternating alignment
//         parentDiv.appendChild(createItemDisplay(mobileData[i], true, alignLeft));
//         alignLeft = !alignLeft; // Toggle alignment for each item
...
//     .then(() => {
//       // All dynamic content has been generated, trigger PDF generation
//       setTimeout(() => {
...
// const creatorID = "660686bb9b6744caa9c7ad1a"
// // console.log("crye in new",creatorID)
...
// //         jsPDF: { unit: 'pt',  format: 'a4', orientation: 'portrait' }
// //     };
// //     html2pdf().set(options).from(ele).toPdf().output('blob').then(function(blob) {
// //         const formData = new FormData();
// //         formData.append('creatorID', creatorID);
// //         formData.append('pdf', blob, 'pdf_file.pdf');
// //         fetch('http://localhost:8080/upload-pdf', { // Your backend endpoint
// //             method: 'POST',
// //             body: formData,
// //         })
// //         .then(response => response.json())
...
//             // pageBreak.style.border = "4px solid red";
...
// // Assuming this script runs in an environment where 'document' is available
